<template>
  <v-container fluid class="mt-md-12 pa-0 d-md-flex justify-center">
    ...
    <div class="px-md-4 px-2 rightDiv fontColor2" style="border-radius: 10px;">
      <v-card flat class="fontColor2">
        ...
        <v-col cols="12" class="d-flex justify-space-between pa-0 mt-3">
          ...
          <v-col class="text-right" style="font-size: 18px">
            <div v-if="lowerPrice" >{{ showPrice }}ETH</div>
            <div v-else class="errorColor">{{ showPrice }}ETH</div>
            <div class="mt-2 btnColor--text">{{ fee }}ETH</div>
          </v-col>
        </v-col>
        <v-col cols="12">
          <v-btn
...
export default {
  ...
  computed: {
    ...mapGetters(["user", "host", "getRate", "config", "getBalanceById"]),
    //余额
    totalPrice() {
      if (this.config) {
